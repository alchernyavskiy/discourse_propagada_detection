...
class XLMRobertaForSequenceClassification(XLMRobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.linear = nn.Sequential(
            nn.Linear(output_dim + config.extra_feature_size, output_dim),
            nn.ReLU(),
        ...
        # classifier like the original XLMRobertaClassificationHead
...
#         )
    ...
    def forward(
        ...
        if extra_features is not None:
            sequence_output = torch.cat([sequence_output, 1.0 * extra_features], dim=-1)
...
# Copied from transformers.models.roberta.modeling_roberta.RobertaForTokenClassification with Roberta->XLMRoberta, ROBERTA->XLM_ROBERTA
class XLMRobertaCustomForTokenClassification(XLMRobertaPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
